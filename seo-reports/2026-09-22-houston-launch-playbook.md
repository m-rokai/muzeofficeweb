...
3. Update the remaining "opening 2026" strings: `app/page.tsx` (Houston feature, FAQ), `app/(marketing)/locations/page.tsx`, `components/layout/site-footer.tsx`, `app/llms.txt/route.ts`, `BRAND.tagline` in `lib/utils/constants.ts`, `houstonLocationFAQs` in `lib/data/faqs.ts`, `CONTACT_INTERESTS` "Houston Waitlist" in `lib/data/contact-interests.ts`, and the Houston blog posts that say "no Houston address service is active yet".
